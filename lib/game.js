...
Game.prototype.makeScene = function(name, sceneData) {
	...
	if (typeof sceneData.onEnter === "string") {
		var script = this.loadScript(sceneData.onEnter);
		if (typeof script === "function") {
			script = script.bind(scene, data);
		}
		scene.onEnter = script;
	}
	if (typeof sceneData.onExit === "string") {
		var script = this.loadScript(sceneData.onExit);
		if (typeof script === "function") {
			script = script.bind(scene, data);
		}
		scene.onExit = script;
...
Game.prototype.installSystems = function(systems, ecs, data) {
	systems.forEach(function(system) {
		this.loadScript(system)(ecs, data);
	}.bind(this));
};
