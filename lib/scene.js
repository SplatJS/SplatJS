...
Scene.prototype.start = function() {
	...
	var scene = this;
	window.requestAnimationFrame(mainLoop.bind(undefined, scene));
};
/**
...
Scene.prototype.adjustTimestamp = function(elapsedMillis) {
...
function mainLoop(scene, timestamp) {
	...
	var totalMillis = 0;
	do {
		var adjustedMillis = scene.adjustTimestamp(elapsedMillis);
		totalMillis += adjustedMillis;

		incrementTimers(scene.timers, adjustedMillis);
		if (!scene.running) {
			return;
		}
		scene.simulationFunc.call(scene, adjustedMillis);
		scene.camera.move(adjustedMillis);
	} while (totalMillis < elapsedMillis - 6);
